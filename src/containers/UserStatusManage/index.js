...
const UserStatusManage = ({ show, handleClose, decision, userID, assingCompanies }) => {
  ...
  const handleRedirect = () => {
    if (decision == 'approve' || decision == 'reject' || decision == 'save') {
      setTimeout(() => {
        closeButton();
  ...
  const userRejectBody = () => (
    <div>
      <h4>{decision == 'reject' ? 'Do you really want to reject!!' : decision == 'approve' ? 'Do you really want to approve!!'
        : decision == 'save' ? 'Do you want to update the data?' : decision == 'active' ? 'Do you really want to activate user!!' : 'Do you really want to deactivate user!!'}</h4>
      {decision == 'reject' &&
  ...
  return (
    <Overlay
      ...
      size="md"
      body={userRejectBody()}
      primary={decision == 'reject' ? 'Confirm Reject' : decision == 'approve' ? 'Confirm Approve' : decision == 'save' ? 'Save' : decision == 'active' ? 'Activate User' : 'Deactivate User'}
