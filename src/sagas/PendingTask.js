...
export function* pendingTasksGetRequest(data) {
  try {
    const response = yield doGet(`${envConfig.apiEndPoints.getPendingTasksList}/${data.currentTab.replaceAll(' ', '')}/${data.currentRole.replaceAll(' ', '')}?page=${data.newPage + 1}&limit=${data.newRowPerPage}`);
